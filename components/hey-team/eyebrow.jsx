...
// .viewport — the terminal session living in this bar is its own
// animation, not the slide's stagger. Purely presentational: Deck owns
// the useTerminalEyebrow call (it needs the same frame timeline to fire
// the content transition and theme wipe at the right instant — see
// deck.jsx), and just hands this component whatever frame comes out.
export const Eyebrow = ({ prompt, command, isTyping, flashSeq = 0 }) => {
  const cursorInPrompt = command.length === 0;
  // Fade only once a sequence has fully settled — freezing solid while
  // isTyping is true keeps the cursor from competing with the text itself
  // (see CLAUDE.md's secondary-action principle).
  const cursorClassName = isTyping ? styles.cursor : `${styles.cursor} ${styles.cursorFade}`;
